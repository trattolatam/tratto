/**
 * Validación de identificadores fiscales (RUT/CUIT/RFC/NIT/RUC/CNPJ) por país.
 *
 * Los algoritmos de dígito verificador de UY, AR, CL, CO, PE y BR fueron extraídos
 * y verificados directamente del código fuente de librerías npm reales y probadas
 * (uruguay-rut-validator, cuit-validator, @fdograph/rut-utilities, colombia-validators,
 * ruc-validate, cpf-cnpj-validator) — no reconstruidos de memoria, para evitar rechazar
 * identificadores reales por error.
 *
 * México (RFC) no tiene un dígito verificador simple de recalcular (usa una tabla de
 * homoclave más compleja administrada por el SAT) — para ese país solo se valida el
 * formato/estructura, no el dígito verificador real.
 */
...
function validateAR(raw: string): TaxIdValidationResult {
  ...
  if (checkDigit === 10) return { formatValid: true, checksumValid: false } // CUIT inválido (no existe dígito 10)
  return { formatValid: true, checksumValid: checkDigit === parseInt(cuit[10]) }
}

// ─── Chile (RUT, hasta 9 dígitos + verificador 0-9 o K) ────────────────────
...
// ─── Brasil (CNPJ, 14 caracteres — soporta el nuevo formato alfanumérico) ──
...
// ─── México (RFC) — solo formato, sin checksum de homoclave ───────────────
...
/**
 * Valida un identificador fiscal según el país. Si el país no tiene validador
 * implementado, devuelve formatValid=true (sin bloquear) y checksumValid=null.
 */
...
// ─── Documento de identidad personal (cédula/DNI) ──────────────────────────
// Solo Chile (mismo algoritmo que el RUT de empresa) y Brasil (CPF) tienen
// dígito verificador confirmado contra código fuente real. Para el resto de
// los países no encontramos una fuente confiable — se valida solo formato.
...
const PERSONAL_ID_VALIDATORS: Record<string, (raw: string) => TaxIdValidationResult> = {
  CL: validateCL, // en Chile el RUT personal usa el mismo algoritmo que el de empresa
  BR: validateCPF,
}
